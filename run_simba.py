...
def get_simba_args(parser):
    ...
    parser.add_argument('--num_attack_iters', type=int, default=1000, help='maximum number of iterations, 0 for unlimited')
...
def run_simba_main(model, mlp_head, data_loader, device, args):
    ...
    print('Launch SimBA Attack!')
    image_size = 224
    attacker = SimBA(model, mlp_head, 'imagenet', image_size)
    ...
    if args.num_attack_iters > 0:
        max_iters = int(min(n_dims, args.num_attack_iters))
    else:
    ...
    for i, (batch, targets) in enumerate(data_loader):
        ...
        adv, probs, succs, queries, l2_norms, linf_norms = attacker.simba_batch(
            images_batch, labels_batch, bool_masked_pos, max_iters, args.freq_dims, args.stride, args.epsilon, linf_bound=args.linf_bound,
            order=args.order, targeted=args.targeted, pixel_attack=args.pixel_attack, log_every=args.log_every)
        if i == 0:
            all_adv = adv
            all_labels = labels_batch
            all_masks = bool_masked_pos
            all_probs = probs
            all_succs = succs
            all_queries = queries
            ...
            all_labels = torch.cat([all_labels, labels_batch], dim=0)
            all_masks = torch.cat([all_masks, bool_masked_pos], dim=0)
        ...
        # savefile = '%s/%s_%s_%d_%d_%d_%.4f_%s%s.pth' % (
            # args.result_dir, prefix, args.model, args.num_runs, args.num_attack_iters, args.freq_dims, args.epsilon, args.order, args.save_suffix)
        if i < 100:
            save_adv_file = '%s/%s_%s_simba_advimage.npy' % (
                args.result_dir, prefix, args.num_attack_iters)
            save_label_file = '%s/%s_%s_simba_label.npy' % (
                    args.result_dir, prefix, args.num_attack_iters)
            save_mask_file = '%s/%s_%s_simba_mask.npy' % (
                    args.result_dir, prefix, args.num_attack_iters)
            np.save(save_adv_file,all_adv.numpy())
            np.save(save_label_file,all_labels.numpy())
            np.save(save_mask_file,all_masks.numpy())
