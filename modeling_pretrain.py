...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def forward_all_features(self, x):
        ...
        if self.fc_norm is not None:
            ...
            fc_norm_x = self.fc_norm(x.mean(1))
            return fc_norm_x
        else:
            return x[:, 0]
    ...
    def forward(self, x, mask):
        mask_x = self.forward_features(x, mask)
        mask_x = self.head(mask_x)

        all_x = self.forward_all_features(x)
        all_x = self.head2(all_x)
        return mask_x, all_x
...
class PretrainVisionTransformerDecoder(nn.Module):
    ...
    def __init__(self, patch_size=16, num_classes=768, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=nn.LayerNorm, init_values=0., num_patches=196,
                 ):
        super().__init__()
        self.num_classes = num_classes
