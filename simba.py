...
class SimBA:
    ...
    def __init__(self, model, mlp_head, dataset, image_size):
        ...
        def get_activation(name, activation):
            ...
            return hook
        hook = self.model.encoder.head2.register_forward_hook(get_activation('encoder_head', self.activation))
    ...
    def get_probs(self, x, bool_masked_pos, y):

        _, _ = self.model(x.cuda(), bool_masked_pos.cuda())
        encoder_fts = self.activation['encoder_head']
        mlp_output = self.mlp_head(encoder_fts).cpu()
        probs = torch.index_select(F.softmax(mlp_output, dim=-1).data, 1, y)
        return torch.diag(probs)
    
    def get_preds(self, x, bool_masked_pos):
        _, _ = self.model(x.cuda(), bool_masked_pos.cuda())
        encoder_fts = self.activation['encoder_head']
        mlp_output = self.mlp_head(encoder_fts).cpu()
        _, preds = mlp_output.data.max(1)
        return preds
    ...
    def simba_single(self, x, y, num_iters=10000, epsilon=0.2, targeted=False):
        ...
        x = x.unsqueeze(0)
        last_prob = self.get_probs(x, bool_masked_pos, y)
        for i in range(num_iters):
            diff = torch.zeros(n_dims)
            diff[perm[i]] = epsilon
            left_prob = self.get_probs((x - diff.view(x.size())).clamp(0, 1), bool_masked_pos, y)
            if targeted != (left_prob < last_prob):
                x = (x - diff.view(x.size())).clamp(0, 1)
                last_prob = left_prob
            else:
                right_prob = self.get_probs((x + diff.view(x.size())).clamp(0, 1), bool_masked_pos, y)
    ...
    def simba_batch(self, images_batch, labels_batch, bool_masked_pos, max_iters, freq_dims, stride, epsilon, linf_bound=0.0,
        ...
        prev_probs = self.get_probs(images_batch, bool_masked_pos, labels_batch)
        preds = self.get_preds(images_batch, bool_masked_pos)
        if pixel_attack:
            trans = lambda z: z
        else:
        ...
        for k in range(max_iters):
            ...
            preds_next = self.get_preds(expanded, bool_masked_pos[remaining_indices])
            ...
            if remaining.sum() == 0:
                ...
                probs_k = self.get_probs(adv, bool_masked_pos, labels_batch)
            ...
            left_probs = self.get_probs(adv, bool_masked_pos[remaining_indices], labels_batch[remaining_indices])
            ...
            right_probs = self.get_probs(adv, bool_masked_pos[remaining_indices], labels_batch[remaining_indices])
            if targeted:
        ...
        preds = self.get_preds(expanded, bool_masked_pos)
        if targeted:
            remaining = preds.ne(labels_batch)
        else:
            remaining = preds.eq(labels_batch)
