...
from engine_for_pretraining import multi_train_one_epoch, multi_evaluate, contrast_multi_train_one_epoch, anamoly_detection, contrast_multi_evaluate
from utils import NativeScalerWithGradNormCount as NativeScaler
import utils
from collections import OrderedDict
from modeling_pretrain import Mlp, SSL_contrast
...
def get_args():
    ...
    parser.add_argument('--ssl_resume', default='', help='resume ssl from checkpoint')
...
def get_Ssl(args):
    ...
    ssl_hidden_dim = 256
    ssl_head = SSL_contrast(
        in_features=768,
        hidden_features=ssl_hidden_dim, 
        out_features=128,
    )
    return ssl_head
...
def main(args):
    ...
    ssl_head_without_ddp = ssl_head


    n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    mlp_n_parameters = sum(p.numel() for p in mlp_head.parameters() if p.requires_grad)
    ssl_n_parameters = sum(p.numel() for p in ssl_head.parameters() if p.requires_grad)
    ...
    print("Ssl = %s" % str(ssl_head_without_ddp))
    print('number of params: {} M'.format(ssl_n_parameters / 1e6))
    ...
    if args.distributed:
        ...
        ssl_head = torch.nn.parallel.DistributedDataParallel(ssl_head, device_ids=[args.gpu], find_unused_parameters=True)
        ssl_head_without_ddp = ssl_head.module

    optimizer = create_optimizer(args, model_without_ddp)
    mlp_optimizer = create_optimizer(args, mlp_head_without_ddp)
    ssl_optimizer = create_optimizer(args, ssl_head_without_ddp)
    ...
    args.resume = args.ssl_resume
    utils.auto_load_model(
        args=args, model=ssl_head, model_without_ddp=ssl_head_without_ddp, optimizer=ssl_optimizer, loss_scaler=loss_scaler)


    if args.eval:
        test_stats = contrast_multi_evaluate(model, ssl_head, mlp_head, data_loader_src, device,  normlize_target=args.normlize_target, log_writer=log_writer,  patch_size=patch_size[0])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = contrast_multi_train_one_epoch(
            model, mlp_head, ssl_head, data_loader_train,
            optimizer, mlp_optimizer, ssl_optimizer, device, epoch, loss_scaler,
            args.clip_grad, log_writer=log_writer,
            start_steps=epoch * num_training_steps_per_epoch,
            lr_schedule_values=lr_schedule_values,
            wd_schedule_values=wd_schedule_values,
        ...
        if args.output_dir:
            if (epoch + 1) % args.save_ckpt_freq == 0 or epoch + 1 == args.epochs:
                utils.contrast_save_model(
                    args=args, ssl_head=ssl_head, ssl_head_without_ddp=ssl_head_without_ddp, ssl_optimizer=ssl_optimizer,
